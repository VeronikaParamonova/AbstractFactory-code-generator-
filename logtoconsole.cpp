...
void LogToConsole::printLog(const std::string &message)
{
    std::cout << message << std::endl; //Напрямую cuot не может вывести QString, из-за разных типов данных. Поэтому используем метод toStdString()

}

void LogToConsole::printLogSlot(const std::string& message)
{
    printLog(message);
}
